...
public class RechargeController extends BaseController {
	...
	public void add(@Validated Recharge recharge, BindingResult br,
		...
		userInfo.setUserMoney(userInfo.getUserMoney() + recharge.getChargeMoney()); //充值后更新用户余额
	...
	/*ajax方式按照查询条件分页查询会员充值信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(@ModelAttribute("userObj") UserInfo userObj,String chargeTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (chargeTime == null) chargeTime = "";
		if(rows != 0)rechargeService.setRows(rows);
		List<Recharge> rechargeList = rechargeService.queryRecharge(userObj, chargeTime, page);
	    /*计算总的页数和总的记录数*/
	    rechargeService.queryTotalPageAndRecordNumber(userObj, chargeTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("userObj") UserInfo userObj,String chargeTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (chargeTime == null) chargeTime = "";
		List<Recharge> rechargeList = rechargeService.queryRecharge(userObj, chargeTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    rechargeService.queryTotalPageAndRecordNumber(userObj, chargeTime);
	    /*获取到总的页码数目*/
	    int totalPage = rechargeService.getTotalPage();
	...
	public String userFrontlist(@ModelAttribute("userObj") UserInfo userObj,String chargeTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (chargeTime == null) chargeTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Recharge/recharge_userFrontquery_result"; 
	}
	...
	public String delete(@PathVariable Integer rechargeId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  rechargeService.deleteRecharge(rechargeId);
	            request.setAttribute("message", "会员充值删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "会员充值删除失败!");
				return "error";
	...
	/*按照查询条件导出会员充值信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(@ModelAttribute("userObj") UserInfo userObj,String chargeTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(chargeTime == null) chargeTime = "";
        List<Recharge> rechargeList = rechargeService.queryRecharge(userObj,chargeTime);
        ...
        for(int i=0;i<rechargeList.size();i++) {
        	...
        	dataset.add(new String[]{recharge.getRechargeId() + "",recharge.getUserObj().getName(),recharge.getChargeMoney() + "",recharge.getChargeTime()});
        }
        /*
